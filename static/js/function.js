...
async function buscarProducto(codigo) {
    ...
    try {
        const response = await fetch(`/api/product?code=${codigo}`);
...
function agregarProducto(product) {
    if (!product || !product.code_product) {
        mostrarError("Respuesta inválida del servidor");
        return;
    }

    // Evitar duplicados
    if (productos.some(p => p.code_product === product.code_product)) {
        mostrarError("Este producto ya está agregado");
        return;
    }

    productos.push(product);

    const precio = parseFloat(product.price_product);

    const tr = document.createElement("tr");
    tr.innerHTML = `
    <td class="oculto">${product.code_product}</td>
...
inputCodigo.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        ...
        console.log("ENTER detectado, buscando:", codigo);
